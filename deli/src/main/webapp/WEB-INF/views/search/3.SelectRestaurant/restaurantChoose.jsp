...
<link rel="stylesheet" type="text/css"
	href="<c:url value="/css/map/restaurantChoose.css"/>">
<script src="<c:url value="/js/map/restaurantChoose.js"/>"
...
<body>
	<center>
		...
		<table id="tableInfo">
			...
			<tr>
				<c:forEach items="${restaurant}" var="restaurant">
					...
					<form name="parentForm2"
						action="<c:url value='/place/restaurant/selectfinish.dlv'/>">
						<th><input type="radio" name="restno"
							value="${restaurant.restno}"
							onclick="radioClick('${restaurant.restno}')" /></th>
		...
		<input type="hidden" name="place_name" value='${place.name}' /> <input
			type="hidden" id="no" values="" />
		<h2 id="h2style">${place.name}(${place.address})
			주변 ${category.foodtype} <input type="submit" value="확인" />
		</h2>
		</form>
		<script>
			let mapContainer = document.getElementById('map'), // 지도를 표시할 div 
			mapOption = {
				center : new kakao.maps.LatLng('${place.pickuplat}',
						'${place.pickuplon}'), // 지도의 중심좌표
				level : 4
			// 지도의 확대 레벨
			};
